...
ミストホースのスキル **Harvest Horse** を、より使いやすくする MOMI 用MODです。騎乗中は広めの範囲で自動収穫・自動発掘を行い、屋内へ入る時は自動で降車します。騎乗できない場所では、徒歩用の狭い収穫補助が動作します。
...
騎乗中に屋内へ続く入口へ向かって移動すると、通常の騎乗／降車切替と同じ処理で降車します。建物、洞窟、階段など、騎乗のまま入れない入口が対象です。降車した次のフレームに、ゲーム本来の処理で入室します。
...
| `auto_dismount_indoors` | `true` | 屋内へ続く入口で自動降車するか。 |
